...
def multilabel_experiment(df: pd.DataFrame, text_col: str, out_folder: Path, do_search: bool = False):
    ...
    for i, idx in enumerate(X_test.index):
        ...
        # fallback to top-1 by score if model predicted nothing
        if not pred_labels:
            try:
...
def main():
    ...
    res = multilabel_experiment(df, DEFAULT_COLUMN, out_folder, do_search=False)
